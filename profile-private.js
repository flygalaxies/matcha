...
import { getAge, validateEmail, validateUser, validateInterest } from './lib';
...
const getCity = (req, res) => {
  const { username } = req.session;
  const latParis = -26.2041;
  const lngParis = 28.0473;
  ...
  getIP((err, ip) => {
    satelize({ ip }, (err, payload) => {
      ...
      geocoder.reverse({
        ...
        res.send({
          city: loc[0].city || 'Johannesburg',
          country: loc[0].country || 'South Africa',
